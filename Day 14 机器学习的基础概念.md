...
1. **输入层**：接收原始数据的地方。每个“神经元”代表输入数据的一个特征。比如，对于一张32x32像素的灰度图片，输入层可能有1024个神经元，每个神经元接收一个像素的亮度值
2. 
3. **隐藏层**：位于输入层和输出层之间，负责对输入数据进行一系列的非线性变换和特征提取。我们“看不到”它们直接处理输入或输出最终结果，所以叫“隐藏”。在基础网络中，可以有一个或多个隐藏层。
4. **输出层**：负责输出网络的最终结果。它的形式和激活函数由任务决定。
