...
### 7. 一个神经元是如何前向传播的？即神经元的具体实现过程
- **步骤1：加权求和（线性变换）**
	
	神经元接收来自上一层的多个输入信号，对每个信号赋予不同的权重（表示重要性），然后求和，再加上一个偏置项：
	
	z=w1x1+w2x2+⋯+wnxn+b=∑ i=1 n wixi+b
	
	其中：
	
	- xi：第 ii 个输入信号（来自上一层的神经元）
	- wi​：对应输入的权重（可学习参数）
	- b：偏置（可学习参数），用于调整神经元的激活阈值
	- z：加权求和的结果，称为**净输入**（net input）

- **步骤2：非线性激活**
	
	将净输入 zz 送入**激活函数** f，得到神经元的最终输出：
	a=f(z)
	
	其中 a 称为**激活值** ，将作为下一层神经元的输入。
...
> 
> **第二隐藏层** = 中级分析员（部件组合部）
> 4. 他们不看原始像素了，而是看初级分析员的报告
> 5. 负责把线条组合成简单部件：比如C员工说“我看到一个‘横线+竖线’的组合，这像是一个拐角”；D员工说“我看到两个弧线，这可能是一个圆圈的一部分”
> 6. 对应：第二层神经元组合低级特征，形成中级特征
>     
> 
> **第三隐藏层** = 高级分析员（整体识别部）
> 7. 他们看中级分析员的报告，组合出完整概念
> 8. E员工说“我看到了一个‘竖线+两个弧线’的组合，这很像数字‘8’的上面部分”；F员工说“我看到了‘横线+竖线’的组合，这很像数字‘7’的结构”
> 9. 对应：更高层神经元识别更抽象、更完整的模式
> 
> 🟠 **输出层** = 最终决策委员会
> 10. 有10位委员，分别对应数字0-9
> 11. 他们听取所有高级分析员的报告，结合自己的判断，最终投票
> 12. 对应数字“7”的委员得到的支持最多，他大声宣布：“我以95%的信心认为，这是数字7！”
> 13. 对应：输出层给出最终预测结果
...
### 16. 什么是损失函数？
也称为代价函数或误差函数，是一个用来衡量**模型预测值与真实值之间差异**的函数。它输出一个非负的实数，数值越小代表模型预测越准确，数值越大代表预测越糟糕
...
|      损失函数       |                      公式                       |                     特点                      |      适用场景       |
| :-------------: | :-------------------------------------------: | :-----------------------------------------: | :-------------: |
|  **均方误差（MSE）**  |                  L = (y^−y)2                  | • 最常用  <br>• 对大误差惩罚更重（平方）  <br>• 梯度随误差增大而增大 | 一般回归问题（房价、温度预测） |
| **平均绝对误差（MAE）** |                   L=∥y^−y∥                    |       • 对异常值更鲁棒  <br>• 梯度恒定，不易被异常值主导        |   数据中有明显异常值时    |
|   **Huber损失**   | L={12(y^−y)2if ∥y^−y∥≤δδ∥y^−y∥−12δ2otherwise​ |    • MSE和MAE的混合体  <br>• 小误差用MSE，大误差用MAE     |  平衡MSE和MAE的优点   |

**生动理解**：

- **MSE**：裁判用“距离的平方”扣分——射箭离靶心10厘米扣100分，离5厘米扣25分，离得越远扣分越狠（吓死人！）
- **MAE**：裁判按实际距离扣分——离10厘米扣10分，离5厘米扣5分，公平但不够警惕大错误
- **Huber**：裁判设定一个阈值（比如8厘米），8厘米内按平方扣分（严格），超过8厘米按线性扣分（防止个别极端坏箭毁掉总分）
...
|              损失函数               |              公式              |                         特点                          |         适用场景         |
| :-----------------------------: | :--------------------------: | :-------------------------------------------------: | :------------------: |
| **二元交叉熵（Binary Cross Entropy）** | L=−[ylog⁡y^+(1−y)log⁡(1−y^)] | • **最主流**  <br>• 输出层配合Sigmoid  <br>• 对数惩罚，错误越离谱惩罚越大 | 几乎所有二分类问题（垃圾邮件、患病检测） |

**生动理解**：

- 真实标签 yy 是1（是猫），模型预测 y^=0.9y^​=0.9：损失 ≈ -log(0.9) = 0.1（很小，接近正确）
- 真实标签 yy 是1，模型预测 y^=0.1y^​=0.1：损失 ≈ -log(0.1) = 2.3（很大，错得离谱）
- 就像老师对选择题打分：选对得分高，选错不仅不给分还倒扣，而且越是自信地选错（预测接近1但实际是0）扣得越狠
...
|                     损失函数                      |          公式          |                         特点                          |          适用场景          |
| :-------------------------------------------: | :------------------: | :-------------------------------------------------: | :--------------------: |
|     **分类交叉熵（Categorical Cross Entropy）**      |  L=−∑j=1Cyjlog⁡y^j​  | • **最主流**  <br>• 输出层配合Softmax  <br>• 真实标签为one-hot向量 |      手写数字识别、图像分类等      |
| **稀疏分类交叉熵（Sparse Categorical Cross Entropy）** | 同上，但输入为整数标签而非one-hot |                • 计算更高效  <br>• 内存占用更小                | 类别数很多时（如1000类ImageNet） |

**生动理解**：

- 10个类别的数字识别，真实标签是“7”（one-hot：第8位为1，其余0）
- 模型输出Softmax概率：如果给“7”分配0.95，损失 ≈ -log(0.95) = 0.05
- 如果给“7”只分配0.3，损失 ≈ -log(0.3) = 1.2
- 就像老师批改单选题，只看你对正确答案分配了多少概率，分配得越少扣分越多


## 监督学习/无监督学习/半监督学习
### 18. 什么是监督学习？
是指训练数据包含**输入特征**和对应的**正确答案（标签）**，模型通过学习这些“问题-答案”对来掌握规律，然后对新的未知数据做出预测

### 19. 什么是无监督学习？
无监督学习的训练数据**没有标签**，只有输入特征。模型需要自己从数据中发现隐藏的结构、模式或规律

### 20. 什么是半监督学习？
半监督学习介于两者之间：训练数据中**少量有标签，大量无标签**。模型利用无标签数据来辅助学习，通常能比只用少量标签数据学得更好
...
## 归一化
### 23. 什么是归一化？
是将原始数据按照一定规则进行缩放，使得各个特征处于相似的数值范围内
...
### 25. 归一化的常用方法有什么？
#### (1). Min-Max 归一化
将原始数据线性变换到指定的区间，通常是 `[0, 1]` 或 `[-1, 1]` 

**公式**：
Xnorm=X−Xmin / ⁡Xmax⁡−Xmin⁡

- 适用于 数据分布有明显边界的情况 + 没什么异常值的
- **缺点**：对异常值敏感，如果数据中存在极端值，缩放后正常值会被压缩到很小的范围

#### (2). Z-score 标准化
将数据转化为均值为0，标准差为1的分布（即标准正态分布）

**公式**：
Xstd=x−μ / σ
其中μ为均值，σ为标准差。

- 适用于数据近似高斯分布的情况，或者 没有明确边界的数据 + 有一点异常值的
- **优点：** 对异常值不那么敏感（因为使用均值和标准差，极端值的影响被分散）
- **注意：** 标准化后的数据不一定在 `[0,1]` 区间，可能存在负数

#### (3). 最大绝对值归一化
将数据缩放到 `[-1, 1]` 区间，缩放因子为每个特征的最大绝对值。

**公式**：
Xnorm=X / max⁡(∣X∣)

- 适用于稀疏数据（如已中心化的数据）或 需要保留零值的情况
- 不改变数据的正负号，且对异常值敏感
...
## 正则化
### 27. 什么是正则化？
是机器学习中用来**防止过拟合**、提高模型**泛化能力**的一组技术。它的核心思想是在模型训练过程中引入额外的约束或惩罚，限制模型的复杂度，使其不过度依赖训练数据中的噪声和偶然模式

- 其数学本质是：在原有的损失函数（如均方误差、二元交叉熵等）的基础上，加入一个与模型参数相关的**惩罚项**，新的目标函数为：
$$
Jnew = Joriginal + λ⋅Ω(θ)
$$
...
- **特点**：
	- 会使得部分权重**变为0**，产生**稀疏**的权重矩阵
	- 相当于自动进行**特征选择**，让不重要的特征权重为0
	- 适合特征维度很高、希望模型简单且可解释的场景

#### (2). L2正则化
- **公式**：Ω(θ)=∑iwi2

即在损失函数中加入所有权重平方的和（通常乘以 1/2 方便求导）

- **特点**：
	- 会使权重**趋近于0但不等于0**，权重分布更平滑
	- 能够防止某些参数过大，让所有参数都比较小且均匀
	- 是神经网络中最常用的正则化形式，通常通过在优化器中设置 `weight_decay` 参数实现


#### (3). Dropout
- **做法**：在训练过程中，随机“丢弃”一部分神经元（即暂时将其输出置为0），每个批次丢弃的比例由超参数决定（如 0.5）

- **原理**：
	- 迫使网络不能过度依赖某些特定的神经元，必须分散学习特征
	- 相当于训练了多个不同的子网络，最后组合起来，起到集成学习的效果
	- 只在训练时使用，测试时所有神经元都参与（但输出要乘以丢弃比例或缩放）

#### 直观解释：
- L1正则化通过在原本的损失函数上加上 **所有权重的绝对值** 来将某些不重要的权重归零，就好像原本有10个权重，现在进行特征选择后便只有5个了，那么权重就变少了，有就是所谓的便稀疏了，即==L1正则化使得权重变得更加稀疏也就是更加简单，自动去除不重要的权重== 
- L2正则化更像是原本就有10个权重，然后最后还是10个权重，数量没变，但是权重大的会匀一部分给权重小的，最终所有的10个权重都会分布的比较均匀，即==L2正则化使得权重分布均匀== 
- Dropout则是通过随机丢弃使得所有的神经元都得到训练，然后使得得到的模型的神经元都能够参与


## 数据清洗
### 29. 什么是数据清洗？
指识别并纠正（或删除）数据集中存在的错误、不一致、不完整、不准确或不相关部分的过程。它是**数据预处理的核心步骤**，目的是提高数据质量，为后续的分析、建模或可视化奠定可靠基础

### 30. 数据清洗的意义在哪里？
- **提高数据质量**：确保数据的准确性、完整性、一致性、唯一性和时效性
- **避免“垃圾进，垃圾出”**：模型和算法的效果高度依赖于输入数据的质量
- **减少误导性结论**：脏数据可能导致错误的业务决策
- **加速后续处理**：干净的数据能简化特征工程和建模步骤

### 31. 数据清洗的方法有哪些？具体实现过程是怎么样的？
#### (1). 处理缺失值
- 探索缺失值
- 删除缺失值
- 填充缺失值
...
#### (2). 处理异常值
- **统计方法**：
    - Z-score：大于阈值（如 3）视为异常
    - IQR（四分位距）：小于 Q1-1.5×IQR 或大于 Q3+1.5×IQR 视为异常

- **处理方式**：删除、截尾（如用上下限替换）、转换（如取对数）、分箱
