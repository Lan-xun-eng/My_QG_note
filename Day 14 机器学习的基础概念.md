...
|      损失函数       |                                               公式                                                |                     特点                      |      适用场景       |
| :-------------: | :---------------------------------------------------------------------------------------------: | :-----------------------------------------: | :-------------: |
|  **均方误差（MSE）**  |                                      L = (y^−y)2L=(y^​−y)2                                      | • 最常用  <br>• 对大误差惩罚更重（平方）  <br>• 梯度随误差增大而增大 | 一般回归问题（房价、温度预测） |
| **平均绝对误差（MAE）** |                                        L=∥y^−y∥L=∥y^​−y∥                                        |       • 对异常值更鲁棒  <br>• 梯度恒定，不易被异常值主导        |   数据中有明显异常值时    |
|   **Huber损失**   | L={12(y^−y)2if ∥y^−y∥≤δδ∥y^−y∥−12δ2otherwiseL={21​(y^​−y)2δ∥y^​−y∥−21​δ2​if ∥y^​−y∥≤δotherwise​ |    • MSE和MAE的混合体  <br>• 小误差用MSE，大误差用MAE     |  平衡MSE和MAE的优点   |
...
|              损失函数               |                            公式                            |                         特点                          |         适用场景         |
| :-----------------------------: | :------------------------------------------------------: | :-------------------------------------------------: | :------------------: |
| **二元交叉熵（Binary Cross Entropy）** | L=−[ylog⁡y^+(1−y)log⁡(1−y^)]L=−[ylogy^​+(1−y)log(1−y^​)] | • **最主流**  <br>• 输出层配合Sigmoid  <br>• 对数惩罚，错误越离谱惩罚越大 | 几乎所有二分类问题（垃圾邮件、患病检测） |
...
|                     损失函数                      |                  公式                   |                         特点                          |          适用场景          |
| :-------------------------------------------: | :-----------------------------------: | :-------------------------------------------------: | :--------------------: |
|     **分类交叉熵（Categorical Cross Entropy）**      | L=−∑j=1Cyjlog⁡y^jL=−∑j=1C​yj​logy^​j​ | • **最主流**  <br>• 输出层配合Softmax  <br>• 真实标签为one-hot向量 |      手写数字识别、图像分类等      |
